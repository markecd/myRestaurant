...
function Miza({ data }) {
  ...
  const handlePostregelClick = async (id) => {
    try {
      ...
      console.error('Error in handlePostregelClick:', error);
    }
  };
  ...
  }, [data[1]]); // Sledi spremembam stanja
    ...
    <div className="miza-container" style={mizaBorder}>
      <div className="row">
        ...
        <div className="col-lg-12">
          {stanjeMize === 'ZASEDENO_NEPOSTREZENO' && pozicija !== 'RECEPTOR' ? (
            <div>
              <Button variant="primary" type="button" className="miza-button" onClick={handleNarociloClick}>
                Naročilo
              </Button>
              <Button variant="primary" type="button" className="miza-button" onClick={() => handlePostregelClick(data[0])}>
                Postregel
              </Button>
            </div>
          ) : (
